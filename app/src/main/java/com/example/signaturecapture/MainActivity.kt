...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mSaveButton!!.setOnClickListener {
            ...
            if (addJpgSignatureToGallery(signatureBitmap)) {
                Toast.makeText(this@MainActivity, "Signature saved into the Gallery", Toast.LENGTH_SHORT).show()
            } else {
                Toast.makeText(this@MainActivity, "Unable to store the signature", Toast.LENGTH_SHORT).show()
            }
            if (addSvgSignatureToGallery(mSignaturePad!!.signatureSvg)) {
                Toast.makeText(this@MainActivity, "SVG Signature saved into the Gallery", Toast.LENGTH_SHORT).show()
            } else {
                Toast.makeText(this@MainActivity, "Unable to store the SVG signature", Toast.LENGTH_SHORT).show()
            }
        }
    }
        ...
        when (requestCode) {
            REQUEST_EXTERNAL_STORAGE -> {
                ...
                if (grantResults.size <= 0 || grantResults[0] != PackageManager.PERMISSION_GRANTED) {
                    Toast.makeText(this@MainActivity, "Cannot write images to external storage", Toast.LENGTH_SHORT)
                        .show()
                }
            }
        }
    ...
    fun addSvgSignatureToGallery(signatureSvg: String): Boolean {
        ...
        try {
            val svgFile =
                File(getAlbumStorageDir("SignaturePad"), String.format("Signature_%d.svg", System.currentTimeMillis()))
